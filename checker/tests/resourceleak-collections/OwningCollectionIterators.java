...
class OwningCollectionIteratorTest {
  ...
  void illegalIteratorRemovalExample(boolean b) {
    ...
    @OwningCollection List<Socket> sockets2 = new ArrayList<Socket>();
    ...
    // at this point, both sockets and sockets2 have @MustCallOnElements("close")
   ...
   * which calls this.iterator.remove(), we would unsoundly not report an error.
   ...
   * report an error.
   */
  void checkThatIteratorParamHasObligation(Iterator<Socket> iter) {
    // :: error: unsafe.iterator.remove
    iter.remove();
  }
   ...
   */
  void safeIteratorParamAccess(Iterator<Socket> iter) {
    try {
      iter.next().close();
      iter.remove();
   ...
   */
  void takeOwningIterator(Iterator<Socket> iter) {
    // :: error: required.method.not.called
    iter.next();
    iter.remove();
  }
  ...
  /*
   * Check that the wilcard type var upper bound for Iterator return types is adjusted correctly.
   * The call-site will think the returned Iterator has no obligations, so the return must be illegal.
   */
  Iterator<?> returnWildcardIterator(@OwningCollection List<Socket> list) {
    Iterator<Socket> iter = list.iterator();
    iter.next();
    close(list);
    return iter;
  ...
  void takeIteratorWithoutTypeArgUpperBound(Iterator<?> iter) {
    iter.next();
    iter.remove();
  }
