...
% To fulfill the must-call obligations of the elements of a collection, it must be fulfilled for all elements at the same time to maintain homogeneity, which can only be achieved in a loop. To ensure a loop operates on all elements of a collection, the Resource Leak Checker puts some constraints on such a loop, as explained in Section~\ref{resource-leak-collections-loops}.
...
The values that must be called on the collection elements are tracked in the \<@MustCallOnElements> type of the collection and the already called methods are tracked in the \<@CalledMethodsOnElements> type. The type system is explained in more detail in Section~\ref{resource-leak-collections-mcoe-cmoe}.
