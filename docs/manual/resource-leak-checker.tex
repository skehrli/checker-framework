...
\subsectionAndLabel{Declaration and assignment of collections of resources}{resource-leak-collections-declaration}
...
\subsectionAndLabel{Assigning elements in a collection of resources}{resource-leak-collections-assign-elements}
...
\subsubsectionAndLabel{Using \<add()> outside a loop}{resource-leak-collections-assign-elements-add}
...
[[What if \<socketList> is a field?  Can there be a method \<void addToSocketList(@Owning Socket s) \{ socketList.add(s); \}>?  I think it should be allowed, but also the type of \<socketList> at method entry will be \<@MustCallOnElements("close")>.  --MS]]
...
exception-free executions of the loop body are considered.  [[Why are only exception-free executions considered?  --MS]]
...
component type. This prevents the need for many manual annotations. However, the user is allowed to write a manual
...
The default type of such a collection after declaration is \<@MustCallOnElements(obligation)>, where \textit{obligation} is the set of \<@MustCall> methods of the component type. The default \<@CalledMethodsOnElements> type is the empty \<@CalledMethodsOnElements({})>.
...
constructor and only once. [[This is except for \<add()> calls right?  --MS]] Thus, the elements of an \<@OwningCollection> field behave like they are final references as well.
The collection stored in the
...
assigned (since \<@OwningCollection> fields are always final).
...
component type, just like for normal methods. The user may write a \MustCallOnElements annotation to override this default.
...
\subsectionAndLabel{\<@OwningCollection> constructor parameters: at the call site}{resource-leak-collections-constructor-call-site}
